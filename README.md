...
* **postfix_smtp_tls_security_level** - the SMTP TLS security level for the Postfix SMTP server (sending). Default for Debian is `dane`, for Red Hat is `may`. For more information visit the [Postfix documentation](http://www.postfix.org/postconf.5.html#smtpd_tls_security_level)
